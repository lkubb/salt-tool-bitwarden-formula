...
We will see a quick setup to configure the ``tool_bitwarden`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_bitwarden`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_bitwarden`` formulas under ``/srv/salt/tool_bitwarden/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_bitwarden/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_bitwarden/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_bitwarden/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_bitwarden/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_bitwarden`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_bitwarden`` formulas under ``/srv/salt/tool_bitwarden/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_bitwarden/parameters/roles

We will define 2 roles:

- ``tool_bitwarden/server``
- ``tool_bitwarden/client``

We create a configuration for the role ``tool_bitwarden/server`` in ``/srv/salt/tool_bitwarden/parameters/roles/tool_bitwarden/server.yaml``:
...
We create another configuration for the role ``tool_bitwarden/client`` in ``/srv/salt/tool_bitwarden/parameters/roles/tool_bitwarden/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_bitwarden/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_bitwarden/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_bitwarden/parameters/dns:domain/  '/srv/salt/tool_bitwarden/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-bitwarden-as-json:
      test.nop:
        - name: {{ bitwarden | json }}
...
.. code-block:: sls
    ...
    bitwarden-config-file-file-managed:
      file.managed:
        - name: {{ bitwarden.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='bitwarden-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ bitwarden.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``bitwarden`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ bitwarden['winner'] }}
